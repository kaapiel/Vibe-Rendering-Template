...
PHXSBridge = {
	 ...
	 */
	openWithACR: function (filePath, xmpString, adobeUserOrientation, pipMethod)  {
		var status = true;
		var savedDialogMode = app.displayDialogs;
		...
		try {
			...
			if (acrDesc != null) {
				descOpen.putObject (stringIDToTypeID("as"), stringIDToTypeID("Adobe Camera Raw"), acrDesc);
			}

			// open the file and return the result (Photomerge and Merge To HDR need the value)
			var resultDesc = app.executeAction (sTID( "open" ), descOpen, DialogModes.ALL);

			if (resultDesc.count === 0) { // when Photoshop fails to parse the file
				status = false;
			}
		}
		catch (err) {
			...
			if (err.number != 8007) { // Don't report user cancelled errors.
				alert('Photoshop scripting error: '+err.message);
			}
		}
	...
	releaseFileReference: function (newTitle) {
		...
		var resultDesc =  executeAction(stringIDToTypeID( "releaseFileReference" ), desc, DialogModes.NO );
	...
	setActiveDocumentTitle: function (newTitle) {
		var ref = new ActionReference();
		ref.putProperty(stringIDToTypeID("property"), stringIDToTypeID("title"));
		ref.putEnumerated(stringIDToTypeID("document"), stringIDToTypeID("ordinal"), stringIDToTypeID("targetEnum"));
		...
		var resultDesc =  executeAction(stringIDToTypeID( "set" ), desc, DialogModes.NO );
