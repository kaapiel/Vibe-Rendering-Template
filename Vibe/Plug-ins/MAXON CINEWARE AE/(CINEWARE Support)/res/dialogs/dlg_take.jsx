...
var buildDialog = function(settings, result) {
	...
	for (var i = 0; i < settings.takes.length; i++) {
		dialog.settings.body.takes.add("item", settings.takes[i]);
	}

	dialog.settings.body.takes.selection = dialog.settings.body.takes.items[settings.settings.selectedTakeIndex];

	dialog.buttons.okButton.onClick = function() {
		result.dialogAction = "ok";

		result.settings = {
			selectedTakeIndex : dialog.settings.body.takes.selection.index
		};
